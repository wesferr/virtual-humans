...
public class WebSocketHubAi : MonoBehaviour
{
    ...
    async Task ConnectAiAsync()
    {
        ...
        while (wsAi == null || wsAi.State != WebSocketState.Open)
        {
            try
            {
                ...
                UnityEngine.Debug.Log("[WS:/ai] Conectando...");
                await wsAi.ConnectAsync(new Uri(serverUrlAi), ctsAi.Token);

                UnityEngine.Debug.Log("[WS:/ai] Conectado!");
                attemptsAi = 0;
                _ = ReceiveLoopAi();
                break;
            }
            catch (Exception ex)
            {
                ...
                UnityEngine.Debug.LogWarning($"[WS:/ai] Falha: {ex.Message}. Retentando em {delay:0.#}s.");
    ...
    async Task ReceiveLoopAi()
    {
        ...
        double t_cli_recv_start;
        double t_cli_recv_end;
        ...
        try
        {
            while (wsAi != null && wsAi.State == WebSocketState.Open)
            {
                ...
                t_cli_recv_start = (double)Stopwatch.GetTimestamp();
                do
                {
                    ...
                    {
                        UnityEngine.Debug.LogWarning("[WS:/ai] Servidor fechou.");
                        await SafeCloseAi();
                ...
                t_cli_recv_end = (double)Stopwatch.GetTimestamp();
                EventHub.SendTextToLs(
                    (long)(t_cli_recv_start * 1e9 / Stopwatch.Frequency),
                    (long)(t_cli_recv_end * 1e9 / Stopwatch.Frequency),
                    (long)1//send = 0 recieve = 1
                );
                UnityEngine.Debug.Log($"Timestamp recieve sent: {t_cli_recv_start}, end: {t_cli_recv_end}");
        ...
        {
            UnityEngine.Debug.LogWarning($"[WS:/ai] Erro recv: {ex.Message}");
            await SafeCloseAi();
            await ConnectAiAsync();
        }
    }
    ...
    async void HandleOutboundBinaryToAi(byte[] data)
    {
        ...
        try
        {
            var t_cli_send_start = Stopwatch.GetTimestamp();
            await wsAi.SendAsync(new ArraySegment<byte>(data), WebSocketMessageType.Binary, true, ctsAi?.Token ?? CancellationToken.None);
            var t_cli_send_end = Stopwatch.GetTimestamp();
            EventHub.SendTextToLs(
                (long)((double)t_cli_send_start * 1e9 / Stopwatch.Frequency),
                (long)((double)t_cli_send_end * 1e9 / Stopwatch.Frequency),
                (long)0//send = 0 recieve = 1
            );
            UnityEngine.Debug.Log($"Timestamp sending sent: {t_cli_send_start}, end: {t_cli_send_end}");

        }
        catch (Exception e) { UnityEngine.Debug.LogError($"[WS:/ai] send bin erro: {e.Message}"); }
    }
    ...
    async void HandleOutboundTextToAi(string s)
    {
        ...
        catch (Exception e) { UnityEngine.Debug.LogError($"[WS:/ai] send text erro: {e.Message}"); }
    }
