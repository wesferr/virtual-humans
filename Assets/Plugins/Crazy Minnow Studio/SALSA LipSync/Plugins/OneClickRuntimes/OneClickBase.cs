				...
				for (int j = 0; j < configuration.oneClickExpressions[exp].components.Count; j++)
				{
					...
					switch (oneClickComponent.type)
					{
						case OneClickComponent.ComponentType.Shape:
							...
							{
								var isSpecificSmrProvided = !string.IsNullOrEmpty(oneClickShapeComponent.specificSmr);
								var isSpecificComponentForWrongSmr = isSpecificSmrProvided && !Regex.IsMatch(requiredSmrs[i].name, oneClickShapeComponent.specificSmr, RegexOptions.IgnoreCase);
								if (isSpecificComponentForWrongSmr)
									continue;
