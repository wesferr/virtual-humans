...
namespace CrazyMinnow.SALSA.OneClicks
{
	...
	public class UmaUepDriverEditorPreview : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			// As of UMA 2.13 a new skeleton requires an UMAGeneratorBase
			// component to guard against adding "ignored" bones to the UMA
			// rig. The UMAGeneratorBase object is not used by SALSA other
			// than to create the skeleton that provides the API for pose
			// generation, particularly to provide an Editor-based preview
			// option.
			skeleton = new UMASkeleton(skeletonRoot, FindUmaGeneratorBase());
		...
		private UMAGeneratorBase FindUmaGeneratorBase()
		{
			var umaGenerator = FindFirstObjectByType<UMAGeneratorBase>() as UMAGeneratorBase;
			...
			GameObject umaGeneratorObject = new GameObject("TempUMAGenerator")
			                  {
				                  // hideFlags = HideFlags.HideAndDontSave
			                  };

			return umaGeneratorObject.AddComponent<UMAGeneratorStub>();
		}
	}
}